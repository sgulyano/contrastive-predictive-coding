...
'''
from data_utils import SortedNumberGenerator, SameNumberGenerator
from maskdata_utils import FaceMaskTileGenerator
...
def train_model(epochs, batch_size, output_dir, code_size, lr=1e-4):
    # Prepare data
    train_data = FaceMaskTileGenerator(batch_size=batch_size, subset='train',
                                       positive_samples=batch_size // 2, rescale=True)

    validation_data = FaceMaskTileGenerator(batch_size=batch_size, subset='valid',
                                            positive_samples=batch_size // 2, rescale=True)
    
    # Prepares the model
    model = network_cpc(image_shape=(32, 32, 3), terms=4, predict_terms=3,
